...
export const POST: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    // lockBracket takes no request body, but callers may still send one
    // (e.g. an empty JSON object) -- draining it unconditionally, before any
    // early return, avoids the same wrangler dev --local keep-alive crash
    // status.ts documents: a response that ends without ever reading the
    // request's own body corrupts the next request replayed over the same
    // connection.
    ...
    if (!hasPermission(session, 'MANAGE_QUEUE')) {
      ...
      // the now-empty stream is a harmless no-op (same as an already-read
      // body in production).
    ...
    const outcome = await stub.lockBracket(sessionId)
    if (!outcome.ok) {
