...
import { listEntrants } from '../../../../../../../worker/repositories/pickleball/tournaments.js'
import { enterTournamentPairSchema } from '../../../../../../../lib/schemas/pickleball/tournaments'
...
export const GET: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    const entrants = await listEntrants(env.PICKLEBALL_DB, params.id)
    return jsonResponse({ entrants }, 200)
  } catch (error) {
...
export const POST: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    const result = enterTournamentPairSchema.safeParse(await request.json().catch(() => null))
    if (!result.success) {
    ...
    const outcome = await stub.enterPair(sessionId, result.data.sessionPairId)
    if (!outcome.ok) {
