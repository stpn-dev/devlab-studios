...
|---|---|---|---|---|
| About (name/title/careerObjectives/shortBio/location/email) | `site_settings` row keyed `profile_about` (JSON blob) | `getSiteSetting`/`setSiteSetting` | `src/data/about.js` | `src/components/admin/ContentManager.jsx` |
| Experience | `experiences` table | `listExperiences`/`replaceExperiences` | `src/data/experiences.js` | `ContentManager.jsx` |
| Certifications | `certifications` table (`id, name, issuer, issued_date, credential_url, badge_image_url, sort_order, status`) | `listCertifications` (from `certifications.js`) | `src/data/about.js` (per the migration's own comment, certs "listed as plain text in `src/data/about.js`... never migrated into D1" for the *original* Make/n8n/Zapier platform badges — the CCNA/Google/OWASP formal certs already flow through the real `certifications` table) | `ContentManager.jsx` |
...
**Certifications:** in the About/document flow's Certifications block, render a small issuer logo next to each entry, resolved from the existing `issuer` column (already populated: "Cisco", "Google", "OWASP", etc.) via a lookup map to vendored `simple-icons` SVGs (same sourcing mechanism as the Tools & Platforms marquee — reuse `scripts/copy-tool-logos.mjs`'s pattern rather than inventing a second one). Confirmed available: Cisco (`#1BA0D7`), Google (`#4285F4`), OWASP (`#000000`). For any issuer without a mapped logo, fall back to a generic Lucide icon (e.g. `BadgeCheck`) — same graceful-degradation pattern used elsewhere on this page. No D1 schema change — this is a presentational lookup keyed on the existing `issuer` string, not a new column.
...
If a task changes `profile_about`'s JSON shape (the About merge) or adds `experiences`/`certifications` rows, verify against a real local D1 instance (`npx wrangler d1 migrations apply devlab-studios-cms --local` first if needed) via `npm run preview`, not just the static fallback — the two paths can silently diverge if only one is tested.
