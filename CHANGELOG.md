...
- **416 new unit tests** (1007 repo-wide), none touching a live external API. Two of them found real defects during development: the schema advertised a `contact_discovery` job type no handler served, which would have dead-lettered as "no handler" and silently lost work; and a shared module-level regex was skipping matches after the first page because `matchAll` copies `lastIndex`.
