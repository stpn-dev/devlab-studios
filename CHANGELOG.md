...
### Added
- **Preview lead-engine shell enabled.** The preview Worker now enables only the Lead Intelligence Engine master switch so its seeded dashboard, settings, sources, campaigns, and manual-import workflow can be exercised against isolated preview data. Discovery, crawling, browser rendering, AI review, tracking, Zoho drafting and sync, and campaign schedules remain disabled.
